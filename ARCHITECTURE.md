...
- `goodreads_import.py` / `shelf_export.py` — the library IN and OUT doors, both pure (stdlib only; `shelf_export` imports `split_series` from its mirror so the two agree on a series tag). `shelf_export` writes the Goodreads-shaped CSV that Goodreads and StoryGraph both import; served by `GET /api/export/library` + the public-profile twin. See CLAUDE.md → "Library export".
...
- `app/*` — pages (server components fetch via SSR). Top-level: `add-book`, `edit-ratings`, `predict` (+ the nested `predict/genres`), `read-queue`, `stats`, `analytics`, `calibration`, `track-record`, `methodology`, `delta-log`, `weights`, `welcome`, `login`, `import`, home. Route groups `fiction/*` + `nonfiction/*` (rankings, tier-list, series, reading, timeline) share `components/views/*View.tsx` (kind-param). (Read-queue is bespoke, not a shared view: fiction's is the top-level `/read-queue`, nonfiction's is `/nonfiction/read-queue`.)
...
3. `test_engine.py` must stay 38/38.
...
- **Verification** — `test_engine.py` 38/38, `python3 scripts/export_static_data.py --check`, and driving the actual affected flow.
