...
_Earlier measurements, for the record: 84.0% / 36.0% on the 125 folds of the
...
whose staleness is keyed on `intervals.engine_hash` (`predict_engine.py` +
`validate_engine.py`). The 2026-09-07 change was to `research_predict.py`, which
only the WIDER `backtest_engine_hash` covers. Two artifacts, two triggers._
